...
if (isset($_GET['q'])) {
    ...
    $usuario_id = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM usuarios WHERE usuario_id = $usuario_id");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
...
if (isset($_POST['regDetalle'])) {
    ...
    if (empty($_POST['actual']) || empty($_POST['nueva'])) {
        ...
        $consulta = mysqli_query($conexion, "SELECT * FROM usuarios WHERE clave = '$actual' AND usuario_id = $id");
        $result = mysqli_num_rows($consulta);
        if ($result == 1) {
            $query = mysqli_query($conexion, "UPDATE usuario SET clave = '$nueva' WHERE usuario_id = $id");
            if ($query) {
                $msg = 'ok';
            }else{
                $msg = 'error';
